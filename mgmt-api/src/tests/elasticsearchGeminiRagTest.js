 ...
 * from Google Gemini to support a RAG system.
 */
...
const createKnowledgeIndex = async () => {
  try {
    const indexName = 'knowledge_base2';
...
const generateSampleKnowledgeData = async () => {
  const sampleDocs = [
    ...
    },
    {
      ...
      content: 'Google Gemini cung cấp API tạo embeddings để chuyển đổi văn bản thành vector biểu diễn ngữ nghĩa. Để sử dụng, bạn cần gọi API embedContent và truyền vào văn bản cần tạo embedding. Kết quả trả về là một vector số thực với số chiều tùy thuộc vào mô hình. Embeddings này có thể được lưu trữ trong Elasticsearch để tìm kiếm ngữ nghĩa.',
  ...
  for (let i = 0; i < sampleDocs.length; i++) {
    ...
    try {
      ...
      console.log(`Generating embedding for document ${i + 1}/${sampleDocs.length}: ${doc.title}`);
      ...
      console.error(`Failed to generate embedding for document ${i + 1}: ${error.message}`);
    }
  }
...
const indexSampleKnowledgeData = async () => {
  try {
    const sampleDocs = await generateSampleKnowledgeData();
    console.log(`Generated ${sampleDocs.length} sample knowledge documents with Gemini embeddings. Indexing to Elasticsearch...`);
    
    // Prepare bulk operations
    const operations = sampleDocs.flatMap(doc => [
      { index: { _index: 'knowledge_base2', _id: doc.id } },
      doc
    ]);
...
/**
 * Perform a vector search using real Gemini embeddings
 */
...
const compareSearchMethods = async (query) => {
  ...
  console.log(`- Vector search (with Gemini embeddings) found ${vectorIds.size} results`);
...
/**
 * Simulate a RAG process with retrieved documents using real Gemini embeddings
 */
const simulateRagProcess = async (userQuery) => {
  console.log(`\n=== Simulating RAG process with Google Gemini for query: "${userQuery}" ===`);
  
  // Step 1: Retrieve relevant documents using vector search with Gemini embeddings
  console.log('\nStep 1: Retrieving relevant documents with semantic search...');
  ...
  console.log('\nStep 2: Formatting contexts for the Gemini LLM...');
  ...
  // Step 3: In a production environment, send query and contexts to Gemini LLM
  // Here we'll simulate the response, but in production you would:
  // - Format a prompt with contexts and the user query
  // - Send to Gemini generative model
...
${contexts.map((ctx, i) => `--- DOCUMENT ${i+1}: ${ctx.title} ---
${ctx.content}
`).join('\n')}

Hãy trả lời câu hỏi sau một cách đầy đủ, chính xác và hữu ích: ${userQuery}
`;

  console.log("Prompt template for Gemini LLM:");
  console.log("---------------------------------------");
  console.log(promptTemplate);
  console.log("---------------------------------------");

  // In a real implementation, you would call Gemini LLM here
  // const geminiResponse = await callGeminiLLM(promptTemplate);
...
/**
 * Implementation of a real call to Gemini LLM (would be used in production)
 */
const callGeminiLLM = async (prompt) => {
  try {
    ...
    return "Tôi xin lỗi, đã xảy ra lỗi khi tạo câu trả lời.";
  }
};

/**
 * Run the Elasticsearch RAG test with real Gemini embeddings
 */
const runElasticsearchRagTest = async () => {
  console.log('Starting Elasticsearch RAG test with Google Gemini embeddings...');
  ...
  console.log('\n=== Simulating complete RAG process with Gemini embeddings ===');
  ...
  console.log('\n=== Elasticsearch RAG test with Gemini embeddings completed ===');
